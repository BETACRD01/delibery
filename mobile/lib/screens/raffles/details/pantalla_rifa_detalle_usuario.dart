...
class _PantallaRifaDetalleUsuarioState
  ...
  Widget _buildBody() {
    ...
    final premiosImagenes = (rifa['premios_imagenes'] as List<dynamic>?) ?? [];
    final premiosParaGaleria = premios.isNotEmpty
        ? premios
        : premiosImagenes.map((imagen) => {'imagen_url': imagen}).toList();
    ...
    final pedidosFaltantes =
        int.tryParse((rifa['pedidos_faltantes'] ?? 0).toString()) ??
    ...
    final premioGanado = rifa['premio_ganado'] is Map<String, dynamic>
  ...
  Widget _buildDetailRow(String label, String value) {
    return Row(
      children: [
        ...
        Expanded(
          child: Text(value, style: const TextStyle(color: Color(0xFF1C1C1E))),
        ),
      ],
    );
  }
  ...
  Widget _imagePlaceholder() {
    return Container(
      color: const Color(0xFFF2F2F7),
      child: const Center(
        child: Icon(CupertinoIcons.photo, size: 48, color: Color(0xFF8E8E93)),
      ),
    );
  }
