...
class PantallaResetearPasswordUsuario extends StatefulWidget {
  ...
  @override
  State<PantallaResetearPasswordUsuario> createState() =>
      _PantallaResetearPasswordUsuarioState();
}

class _PantallaResetearPasswordUsuarioState
    extends State<PantallaResetearPasswordUsuario> {
  ...
  Future<void> _buscar() async {
    ...
    try {
      final data = await _usuariosApi.buscarUsuarios(
        search: _searchController.text.trim(),
      );
      final resultados = data['results'] as List? ?? [];
      if (resultados.isEmpty) {
        setState(
          () => _errorBusqueda = 'No se encontró ningún usuario con ese dato',
        );
      } else {
        setState(
          () => _usuarioSeleccionado = resultados.first as Map<String, dynamic>,
        );
      }
        ...
        child: Column(
          ...
          children: [
            ...
            if (_usuarioSeleccionado != null)
              _buildUsuarioCard(_usuarioSeleccionado!),
            const SizedBox(height: 12),
            Expanded(
              child: Form(
                key: _formKey,
