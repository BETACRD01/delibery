...
class UbicacionService {
  ...
  void _log(String message, {Object? error, StackTrace? stackTrace}) {
    developer.log(message, name: 'UbicacionService', error: error, stackTrace: stackTrace);
  }
  ...
  Future<Position?> obtenerUbicacionActual() async {
    try {
      ...
      try {
        ...
        _log('Fallo al obtener ubicacion en vivo, intentando ultima conocida', error: e);
  ...
  Future<bool> iniciarEnvioPeriodico({Duration? intervalo}) async {
    try {
      ...
      _log('Envio periodico iniciado (Intervalo: ${intervaloPeriodico.inSeconds}s)');
      return true;
    } catch (e, stackTrace) {
  ...
  Future<bool> iniciarRastreoTiempoReal({
    ...
    try {
      ...
      _streamSubscription = Geolocator.getPositionStream(locationSettings: settings)
  ...
  Future<void> _enviarUbicacionAlServidor(Position position) async {
    try {
      ...
      if (e.statusCode == 401) {
        ...
        if (e.isNetworkError) {
          _log('Sin conexion, ubicacion no enviada (se reintentara automaticamente)');
          return;
        }
  ...
  void imprimirEstado() {
    ...
    _log('Lat/Lon: ${_ultimaUbicacion?.latitude ?? "N/A"}, ${_ultimaUbicacion?.longitude ?? "N/A"}');
  }
...
enum ModoUbicacion {
  ...
  String get nombre {
    switch (this) {
      case ModoUbicacion.ninguno: return 'Ninguno';
      case ModoUbicacion.periodico: return 'Periodico';
      case ModoUbicacion.tiempoReal: return 'Tiempo Real';
    }
  }
}
