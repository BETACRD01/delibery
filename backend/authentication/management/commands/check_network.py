...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Red detectada: {config_red['nombre']}")
            )
            ...
            if not config_red["valida"]:
                self.stdout.write(
                    self.style.WARNING("\nRed no válida, usando configuración fallback")
                )
            ...
            self.stdout.write(
                f"  CSRF_TRUSTED_ORIGINS: {len(settings.CSRF_TRUSTED_ORIGINS)} orígenes"
            )

            if hasattr(settings, "CORS_ALLOWED_ORIGINS"):
                self.stdout.write(
                    f"  CORS_ALLOWED_ORIGINS: {len(settings.CORS_ALLOWED_ORIGINS)} orígenes"
                )
            ...
            self.stdout.write("\n" + "=" * 70)
            self.stdout.write(self.style.SUCCESS("erificación completada"))
            self.stdout.write("=" * 70 + "\n")

        except ImportError:
            self.stdout.write(
                self.style.ERROR(" Error: network_detector.py no encontrado en utils/")
            )
            self.stdout.write(
                self.style.WARNING(
